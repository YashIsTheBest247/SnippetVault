...
def create_snippet(payload: SnippetIn):
    ...
    with db.get_conn() as conn:
        cur = conn.execute(
            "INSERT INTO snippets (title, language, code, tags, pinned, created_at, updated_at) VALUES (?, ?, ?, ?, 0, ?, ?)",
            (payload.title, payload.language, payload.code, db.normalize_tags(payload.tags), ts, ts),
        )
...
def toggle_pin(snippet_id: int):
    with db.get_conn() as conn:
        ...
        conn.execute("UPDATE snippets SET pinned = ?, updated_at = ? WHERE id = ?", (new_val, db.now_iso(), snippet_id))
        row = conn.execute("SELECT * FROM snippets WHERE id = ?", (snippet_id,)).fetchone()
    return row_to_snippet(row)
